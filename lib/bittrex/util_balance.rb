  ...
  def self.get_balance(usdt_base=0)
    ...
    balances.each do |k,bal_avail|
      balance, available = bal_avail
       ...
       if k==base_crypto
        res[k]={ btc:balance, usdt:balance*usdt_base, bid:1, ask:1}
        next
       end
       pair = pair(k)
...
##############  
  ...
  def self.get_balance_for_site ##used in ---controllers/trade.rb
    ...
    ticks = DB[:my_ticks].to_hash(:name,[:bid,:ask])
    pair_settings =  DB[:my_trade_pairs].filter( Sequel.lit("pid=? and name in ? ", get_profile, curr_) ).to_hash(:name, [:operation_amount,:center_price])
    ...
    balances.each do |k,bal_avail|
      ...
      next if !pair_settings[mname]
      
      center_price = pair_settings[mname][1]
      ...
      operation_amount = pair_settings[mname][0]
  ...
  def self.get_simul_tokens ##used in ---controllers/trade.rb
    ...
    simul_curr.each do |curr|
      ...
      history = []
      if prices[pair]
        history = prices[pair][:price_history]
      end

      res << { pair:pair, last_bid:bid, last_ask:ask, diff:diff_str, price_history:history }
