...
####
  ...
  def self.table_tick
    ...
    lines_tr=[]
    lines_tr<< "<th>balance</th> <th>available</th> <th>USDT bal</th> <th>BID</th> <th>ASK</th> <th>diff</th> <th>pair</th>"
    ...
    data.each do |dd|
      ...
      line += "<td style='width:25%;'> #{btn_buy_code} #{txt_code} #{btn_sell_code} #{btn_rate_code}</td>"   
      line += "<td style='width:25%;'>#{dd[:orders_info]}</td>" 
      lines_tr<<"<tr>#{line}</tr>"
    end
