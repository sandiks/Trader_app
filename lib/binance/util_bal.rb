...
module Binance
  ...
  class BalanceUtil
    ...
    def self.btc_usd
      usd_bid = DB[:my_ticks].filter(name:'BTCUSDT').select(:bid,:ask).first
    end

    def self.my_btc
       DB[:wallets].filter(currency:'BTC').first[:balance] 
    end
  
    def self.get_bid_ask_from_tick(symb) 
      DB[:my_ticks].filter(name:symb).select_map([:bid,:ask]).first
    end    
     
    def self.get_all_bid_ask
      DB[:my_ticks].to_hash(:name,[:bid,:ask])
    end
    ...
    def self.add_to_simul(mname)
      p "add_to_simul"
      ask = BalanceUtil.get_bid_ask_from_tick(mname)[1] rescue 0
      DB[:simul_markets].filter(pid:get_profile, name:mname).delete
      DB[:simul_markets].insert({pid:get_profile, name:mname, ppu:ask, buy_time: date_now(0)})
    end
